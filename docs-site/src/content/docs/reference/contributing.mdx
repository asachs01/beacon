...
Create a `.env` file in the project root (see [Configuration](/docs/customization/configuration/) for all options):
...
See [Themes > Creating custom themes](/docs/customization/themes/) for the complete `Theme` interface and guidelines.
