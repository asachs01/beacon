...
Family command center for wall-mounted displays"
order: 2
---
...
All Beacon styles use CSS custom properties (see [Themes](/docs/customization/themes/) for the complete list), so most visual customizations can be achieved by overriding the `--*` properties on `:root` rather than targeting specific class names.
